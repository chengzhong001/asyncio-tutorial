...
#let heading_style(term, color: blue) = {
  text(color, box[#term])
}
...
+ `CPU` 密集型的工作主要使用计算机处理器，而 `IO` 密集型的工作主要使用网络或其他输入/输出设备。`asyncio` 主要帮助我们使 `IO` 密集型工作并发执行，但也提供了使 `CPU` 密集型工作并发进行的 `API`。

+ 进程操作系统资源分配的基本单元，线程是操作系统调度操作 `CPU` 的基本单元。

+ 在使用非阻塞套接字时，可指示操作系统告诉我们何时有数据传入，而不是在等待数据传入时停止应用程序。这是允许 `asyncio` 仅使用单个线程实现并发的部分原因。

+ `asyncio` 应用程序的核心，事件循环会一直循环下去，寻找 `CPU` 密集型任务，同时暂停正在等待 `IO` 的任务。注意：这里的暂停，不是说任务就完全处于静止状态了，而是指不需要在该任务上浪费 `CPU` 了。因为 `IO` 操作一旦发起，就由操作系统接管，程序可以去执行其它任务了，但 `IO` 操作的过程是不会中断的，因为它不耗费 `CPU`。而当 `IO` 操作完成后，会收到操作系统通知，然后可以再切换回该任务，继续执行该任务剩余的 `CPU` 密集代码。这样就保证了 `CPU` 的利用率，其实说白了核心就一句话：在任务陷入 `IO` 阻塞的时候，不要傻傻地等待，而是去执行其他的任务。
